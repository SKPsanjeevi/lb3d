...
!> surface of walls
module lbe_md_rock_dlvo_module
#ifdef MD
    ...
    public input_rock_dlvo,local_rock_potential_dlvo,particle_rock_potential_dlvo&
         &,rock_f_interaction_dlvo,setup_rock_dlvo
    ...
    namelist /md_rock_dlvo/ epsilon,sigma

contains

    !> read namelist  /md_rock_dlvo/  from input file
    subroutine input_rock_dlvo
        integer ierror

        call log_msg_md_hdr("Reading MD R DLVO input")
        ...
        if (myrankc.eq.0) then
           ...
           read (unit=md_input_file_unit,nml=md_rock_dlvo,err=100)
           close (unit=md_input_file_unit,err=100)
           !call log_msg_md('read /md_rock_dlvo/ from file "'//trim(inp_file)&
           !     &//'.md"',.false.)
           !write (6,nml=md_rock_dlvo)
        end if
        ...
        if ( arg_input_dfile_set ) then
          ...
          read(UNIT = input_dfile_unit, NML = md_rock_dlvo, IOSTAT = ierror)
          if (ierror .ne. 0) then
...
100     continue
    ...
    end subroutine input_rock_dlvo

    subroutine local_rock_potential_dlvo(N,pot)
    ...
    end subroutine local_rock_potential_dlvo
    ...
    !> position \c pos.
    real(kind=rk) function particle_rock_potential_dlvo(rock_state,pos)
        ...
        particle_rock_potential_dlvo = pot
    end function particle_rock_potential_dlvo

    subroutine rock_f_interaction_dlvo(N)
        type(lbe_site),intent(in) :: &
             &N(1-halo_extent:,1-halo_extent:,1-halo_extent:)
        integer i,ii,j,x,y,z
    ...
    end subroutine rock_f_interaction_dlvo

    subroutine setup_rock_dlvo
        ...
        ! polydispersity yet
        if (polydispersity) call error_md("rock='dlvo' does not yet "&
    ...
    end subroutine setup_rock_dlvo
...
#endif
end module lbe_md_rock_dlvo_module
