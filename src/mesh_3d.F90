...
! -------------------------------------------------------------------------
! create a 3-d mesh of processors for physical volume (xprd,yprd,zprd)

      subroutine mesh_3d(node,ncores,idimension,xprd,yprd,zprd, &
     &     pgrid,me,mpart)
      use mpi
      implicit none
...
! argument variables
      ...
      integer node,ncores,idimension
...
! map procs into 3-d grid that most closely matches box volume

      if (pgrid(1).eq.0) call proc2box3d(ncores,idimension, &
     &     xprd,yprd,zprd,pgrid(1),pgrid(2),pgrid(3))
      
! determine where I am and who are my neighboring procs in 3-d proc grid
! order axes so as to agree with old version of code
! this ordering puts procs along x-axis first, then y, then z
...
! ----------------------------------------------------------------------
! assign ncores to 3-d xprd,yprd,zprd box so as to minimize surface area
! if idimension = 2, enforce pz = 1
! return 3-d px,py,pz grid of procs

      subroutine proc2box3d(ncores,idimension,xprd,yprd,zprd,px,py,pz)
!gjp including this for subroutine error
      use lbe_log_module, only: error
      implicit none

! argument variables

      integer ncores,idimension
      real*8 xprd,yprd,zprd
      integer px,py,pz
...
! loop thru all possible factorizations of ncores
! surf = surface area of a proc sub-domain

      ipx = 1
      do while (ipx.le.ncores)
        if (mod(ncores,ipx).eq.0) then
          nremain = ncores/ipx
          ipy = 1
     ...
     &             (idimension.eq.2.and.ipz.eq.1))) then
      ...
      if (px*py*pz.ne.ncores) call error('Bad result in proc2box3d')
...
! ----------------------------------------------------------------------
! assign ncores to 2d nx,ny grid so as to minimize surface area
! return 2d px,py grid of procs

      subroutine proc2grid2d(ncores,nx,ny,px,py)
!gjp including this for subroutine error
      use lbe_log_module, only: error
      implicit none
...
! innermost if test minimizes surface area and surface/volume ratio
     ...
     &         boxx*boxy.gt.bestboxx*bestboxy)) then
      ...
      if (px*py.ne.ncores) call error('Bad result in proc2grid2d')
