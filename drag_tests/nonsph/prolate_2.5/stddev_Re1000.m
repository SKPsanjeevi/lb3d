...
%hFig=figure('units','inches','position',[4 4 8 5]);
% colormap(hFig, jet);
...
for i=1:size(deg,2)
    ...
    last=size(num,1);
    begin=last-diff;
    clmean=mean(num(begin:last,1))/(0.5*pi*R^2*v^2);
    CDmean=mean(num(begin:last,2))/(0.5*pi*R^2*v^2)
    clmean=mean(num(begin:last,3))/(0.5*pi*R^3*v^2);
    
    cdsd(3,i) = sqrt(mean((CD(begin:last)-CDmean).^2))/CDmean*100;
