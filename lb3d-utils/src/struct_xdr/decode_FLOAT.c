...
{                      
   ...
   (stats->min)=(stats->min2)=(stats->min3)= DBL_MAX;
   (stats->max)=(stats->max2)=(stats->max3)=-DBL_MAX;
 ...
 //         xdr_double(&xdrs, &a); 
	...
	//  a, dat->data[i][j][k]); 
	    ...
	    if(params->owd) {
	       ...
	       if(b > stats->max2)  stats->max2 = (double) b;
	       if(b < stats->min2)  stats->min2 = (double) b;
	    }
	    ...
	    if(params->arr) {
	       ...
	       xdr_float(&xdrs, &c);
               dat->data3[i][j][k] = (double) c;
               if(c > stats->max3)  stats->max3 = (double) c;
               if(c < stats->min3)  stats->min3 = (double) c;
            }
