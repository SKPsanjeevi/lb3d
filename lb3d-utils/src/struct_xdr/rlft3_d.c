...
void rlft3_d(double ***data, double **speq, unsigned long nn1, unsigned long nn2,
	     unsigned long nn3, int isign)
{
        void fourn_d(double data[], unsigned long nn[], int ndim, int isign);
	...
	if (1+&data[nn1][nn2][nn3]-&data[1][1][1] != nn1*nn2*nn3)
		nrerror("rlft3_d: problem with dimensions or contiguity of data array\n");
	c1=0.5;
	c2 = -0.5*isign;
	...
	nn[3]=nn3 >> 1;
	if (isign == 1) {
		fourn_d(&data[1][1][1]-1,nn,3,isign);
		for (i1=1;i1<=nn1;i1++)
