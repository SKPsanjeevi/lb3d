...
int main (int argc, char *argv[]){
  ...
  int       width;  /* width */
  int       widthr, widthl, ii, jj, kk, i_p, j_p, k_p, t, tmax;
...
//		if (i+ii>=0 && j+jj>=0 && k+kk>= 0 && i+ii<=nx-1 && j+jj<=ny-1 && k+kk<=nz-1) {
		  if ( pow(radious,0.2e1) >= f_sphere((float)i+(float)ii, (float)j+(float)jj, (float)k+(float)kk, i, j, k) ) {
                    i_p = (i+ii) - (nx-1)*round(float(i+ii)/float(nx-1)-0.5);
                    j_p = (j+jj) - (ny-1)*round(float(j+jj)/float(ny-1)-0.5);
                    k_p = (k+kk) - (nz-1)*round(float(k+kk)/float(nz-1)-0.5);
//                    printf("%d\t%d\t%d\t%d\t%d\t%d\n", i+ii, j+jj, k+kk, i_p, j_p, k_p);
		    pos[i_p][j_p][k_p] = colour; 
//		    pos[i+ii][j+jj][k+kk] = colour; 
		  }
//		}
	      }
...
//    Periodic sphere
	...
	for ( j=wj-width; j<=wj+width; ++j) {
	 for ( i=wi-width; i<=wi+width; ++i) {
	   if( pow(radious,0.2e1) >= f_sphere(sx, sy, sz, i, j, k) ) {
             i_p = i - (nx-1)*round(float(i)/float(nx-1)-0.5);
             j_p = j - (ny-1)*round(float(j)/float(ny-1)-0.5);
             k_p = k - (nz-1)*round(float(k)/float(nz-1)-0.5);
	     pos[i_p][j_p][k_p] = colour; 
	   }
	 }
	}
