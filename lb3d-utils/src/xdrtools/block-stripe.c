...
The block tool generates xdr domain
...
If a striped wall "stripe" is being set, this crosses the defined domain and its parameters are: 
     1 3 1    # starting point (int)
     1 4 1    # ending point (int)
     0 4 0    # wall growing direction (int)
#  in the 'wall growing direction'
#  can only be a vector with one component different from zero, 
#  this no-zero component is the direction of the wall's width.
...
If a ring is being set, a ring is being build, 
this ring can only be oriented in the direction of the axis x, y, and z.
Its parameters are:
     5.2 7.2 10.2   # starting center point (float)
     0 0 2         # direction an length (int)
#  this last parameter can only be a vector with one component different from zero.
     3.2            # ring inner radius (float)
     4.2            # ring outer radius (float)
     0.0            # colour (float)
...
int main (int argc, char *argv[]){
  ...
  int       wsi, wsj, wsk; /* stripe start position */
  int       wei, wej, wek; /* stripe end position */
  ...
  float     ring_rout;  /* outer radius */
  float     ring_rin;  /* inner radius */
  float     width1, width2; /* for loop*/
...
/*start of stripe*/
...
} else if (strcmp(shape,"stripe") == 0){

      fscanf(f0, "%d %d %d", &wsi, &wsj, &wsk);
      fscanf(f0, "%d %d %d", &wei, &wej, &wek);
      fscanf(f0, "%d %d %d", &vg_i, &vg_j, &vg_k);
      fscanf(f0, "%f", &colour);
      wll += 1;   
      printf("stripe %d: %d %d %d %d %d %d %d %d %d %f\n", wll, wsi, wsj,wsk, wei, wej, wek, vg_i, vg_j, vg_k, colour);
	...
	printf("Attention, wrong definition of the stripe %d\n",wll);
	exit(1);
      }   
	...
	printf("Attention, wrong definition of the circ_column %d\n",sqr_clm);
	exit(1);
      }
    /* end of circ_column */

} else if ( strcmp(shape,"ring") == 0 ) { 
      fscanf(f0, "%f %f %f", &sx, &sy, &sz);
      fscanf(f0, "%d %d %d", &vg_i, &vg_j, &vg_k);
      fscanf(f0, "%f", &ring_rin);
      fscanf(f0, "%f", &ring_rout);
      fscanf(f0, "%f", &colour);
      ring += 1;
      printf("ring %d: %f %f %f %d %d %d %f %f %f\n", ring, sx, sy, sz, vg_i, vg_j, vg_k, ring_rin, ring_rout, colour); 
	...
	if ( vg_i>0 ) {
	  for ( k=wk-width; k<=wk+width; ++k ) {
		...
		jj = f_domain(j, ny);
		kk = f_domain(k, nz);
		if ( pow(ring_rout,0.2e1) >= f_circ(sy, sz, jj, kk) && pow(ring_rin,0.2e1) <= f_circ(sy, sz, jj, kk)) {
		  pos[ii][jj][kk] = colour; 
		}
	      }
	    }
	...
	if ( vg_j>0 ) {
	  for ( k=wk-width; k<=wk+width; ++k ) {
		...
		jj = f_domain(j, ny);
		kk = f_domain(k, nz);
		if ( pow(ring_rout,0.2e1) >= f_circ(sx, sz, ii, kk) && pow(ring_rin,0.2e1) <= f_circ(sx, sz, ii, kk) ) {
		  pos[ii][jj][kk] = colour; 
		}
	      }
	    }
	...
	if ( vg_k>0 ) {
	  for ( k=wk; k<=wk+vg_k-1; ++k ) {
		...
		jj = f_domain(j, ny);
		kk = f_domain(k, nz);
		if ( pow(ring_rout,0.2e1) >= f_circ(sx, sy, ii, jj) && pow(ring_rin,0.2e1) <= f_circ(sx, sy, ii, jj) ) {
		  pos[ii][jj][kk] = colour; 
		}
	      }
	    }
	...
	printf("Attention, wrong definition of the ring %d\n",sqr_clm);
	exit(1);
      }
    /* end of ring */
    }
