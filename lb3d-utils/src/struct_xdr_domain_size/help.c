...
print_help(char executable[40])
{
   printf("\n\n\t<<<<<<<<<<<<<<< HELP ON struct_xdr"
	  ...
	  "\t* FILETYPE can be 'colour', 'sur', or 'owd'.\n"
	  "\t  (colour or surfactant density respectively).\n"
	  ...
	  "\tPLEASE NOTE:\n\n"
	  "\t* There are two versions of this program, which are obtainable\n"
	  "\t  by compiling with Makefile or Makefile_B, respetively. Please\n"
	  "\t  read the README file to learn about their diferences.\n"
	  ...
	  "\t* File .siz contains different fields depending on which version\n"
	  "\t  of the program, ie. Makefile or Makefile_B, is being used:\n"
	  "\t  ~~~~ Makefile ~~~~\n"
	  "\t\tts k1 errk1 k2 errk2 R1 errR1 R2 errR2 min max avg\n"
	  "\t  ~~~~Makefile_B~~~~\n"
	  "\t\tts k1 errk1 k2 errk2 R1 errR1 R2 errR2 Rx errRx Ry errRy Rz errRz min max avg\n"
	  "\t  where ts is the time step, k? are the ?th moments of\n"
	  "\t  S(k) with respect to k, R1 is 2PI/k1, R2 is 2PI/sqrt(k2),\n"
	  "\t  Ri, with i=x,y,z, are 2PI/sqrt(ki_2), where ki_2 is the second moment of\n"
	  "\t  S(k) with respect to ki, which is the i-th component of vector k.\n"
	  "\t  Finally, err?? are the relevant uncertainties derived from spherical or \n"
	  "\t  lattice averaging.\n" 
	  "\t* The code can be uncommented to give file .lsz containing (ln k,ln R1) data pairs.\n"
