...
int Gitterpunkt(int jj,int gg);
int main()
{
    const int Nz=40;
    const int Nta=154000;
    const int Nt=154000;
    //const int Nt=0;
    const int NT=6258;
    const int dt=100;
    ...
    string datei1st="md-cfg_absorb01_t00000000.asc";
    const int TNr=15;
    const int XNr=0;
    const int YNr=1;
    const int ZNr=2;
    ...
    for(int t=Nta; t<(Nt+1); t++)
    {
        if(t%dt==0)
	  {
                ...
                while (! colfile.eof())
		{
	        ...
	        for(int iz=0; iz<(Gz); iz++)
                {
                    for(int iy=0; iy<(Gy); iy++)
                    {
                        if(Col[ix][iy][iz]>0)NColP+=1;
                        if(Col[ix][iy][iz]<0)NColN+=1;
                        if(Col[ix][iy][iz]<0)
			{
                            int jx;
                            int jy;
                            ...
                            nachbar=false;
                            neueKugel=true;
                            jx=Gitterpunkt(ix-1,Gx);
                            jy=Gitterpunkt(iy-1,Gy);
                            jz=Gitterpunkt(iz-1,Gz);
                            if(Col[jx][jy][jz]<0)if(KNr[jx][jy][jz]!=0)if(neueKugel)
			    {
                                KNr[ix][iy][iz]=KNr[jx][jy][jz];
                                neueKugel=false;
                            }
                            jx=Gitterpunkt(ix-1,Gx);
                            jy=Gitterpunkt(iy-1,Gy);
                            jz=Gitterpunkt(iz,Gz);
                            if(Col[jx][jy][jz]<0)if(KNr[jx][jy][jz]!=0)if(neueKugel)
			    {
                                KNr[ix][iy][iz]=KNr[jx][jy][jz];
                                neueKugel=false;
            ...
            //cout<<KuNr<<endl;
            cout<<NColN<<"    "<<NColP<<endl;
            //for(int ix=0; ix<(Gx); ix++)cout<<ix<<"    "<<Col[ix][20][20]<<"    "<<KNr[ix][20][20]<<endl;


            for(int iy=0; iy<(100*Gy); iy++)
            {
                NKu[iy]=0;
            }
            for(int ix=0; ix<(Gx); ix++)for(int iy=0; iy<(Gy); iy++)for(int iz=0; iz<(Gz); iz++)
            {
                int i=KNr[ix][iy][iz];
                if(KNr[ix][iy][iz]!=0)NKu[i]+=1;
            }
            NKuK10=0;
            NKuGl10=0;
            NKuG10=0;
            ...
            for(int iy=0; iy<(100*Gy); iy++)
            {
                if(NKu[iy]>0)if(NKu[iy]<10)NKuK10+=1;
                if(NKu[iy]==10)NKuGl10+=1;
                if(NKu[iy]>10)NKuG10+=1;
                if(NKu[iy]>0)NKuTest+=1;
            }
            //cout<<NKuK10<<"    "<<NKuGl10<<"    "<<NKuG10<<"    "<<NKuTest<<endl;
            ungleich=true;
            Nungleich=0;
            while(ungleich)
	    {
                    ...
                    if(KNr[ix][iy][iz]>0)
	 	    {
                        ...
                        //if(KNr[jx][jy][jz]!=0)
		        //{
                            //if(KNr[ix][iy][iz]!=KNr[jx][jy][jz])
			    //{
                                //ungleich=true;
                                //KNr[ix][iy][iz]!=KNr[jx][jy][jz];
                            //}
                        //}
                        if(KNr[jx][jy][jz]!=0)if(KNr[ix][iy][iz]!=KNr[jx][jy][jz])
		        {
                            ungleich=true;
                            //KNr[ix][iy][iz]=KNr[jx][jy][jz];
                            NNungleich+=1;
                            KNrAlt=KNr[jx][jy][jz];
                            KNrNeu=KNr[ix][iy][iz];
                            for(int kx=0; kx<(Gx); kx++)for(int ky=0; ky<(Gy); ky++)for(int kz=0; kz<(Gz); kz++)
			    {
                                if(KNr[kx][ky][kz]==KNrAlt)KNr[kx][ky][kz]=KNrNeu;
                            }
                            //for(int kx=0; kx<(Gx); kx++)for(int ky=0; ky<(Gy); ky++)for(int kz=0; kz<(Gz); kz++)
			    //{
                                //jjx=Gitterpunkt(kx,Gx);
                                //jjy=Gitterpunkt(ky,Gy);
                                //jjz=Gitterpunkt(kz,Gz);
                                //if(KNr[jjx][jjy][jjz]==KNrAlt)KNr[jjx][jjy][jjz]=KNrNeu;
                            //}
                ...
                cout<<Nungleich<<"    "<<NNungleich<<endl;
            }
                        for(int ix=0; ix<(Gx); ix++)cout<<ix<<"    "<<Col[ix][20][20]<<"    "<<KNr[ix][20][20]<<endl;
            for(int iy=0; iy<(100*Gy); iy++)
            {
                ...
                if(NKu[iy]>=NKuC)
		{
            ...
            cout<<NKuK10<<"    "<<NKuGl10<<"    "<<NKuG10<<"    "<<NKuCoG<<"    "<<NKuTest<<endl;
            EffNKu[0]=0;
            
            for(int nT=0; nT<NT; nT++)TeilchenAnT[nT]=0;
            for(int i=0; i<(100*Gy); i++)
            {
                if(KuCoG[i])
		{
                    ColM=0;
            ...
            Colrsy=Colrsy/ColM;
            Colrsz=Colrsz/ColM;
            //cout<<Colrsx<<"    "<<Colrsy<<"    "<<Colrsz<<endl;
            ColrsxI=int(Colrsx+0.5);
            ColrsyI=int(Colrsy+0.5);
            ColrszI=int(Colrsz+0.5);
