...
int main()
{
    ...
    double my=0;
    double cy=0;
    const string lb_gr_out_file="write here your output name";
    const string lbID="0170717708";
    const string tstep="00100000";
    string filemd="";
    string filecol="";
    string filemdpart1="";
    string filecolpart1="";
        ...
        while (! colfile.eof())
	{
            ...
            for(int i=0; i<Ni; i++)
	    {
	        ...
	        if(gy<Gy)
		{
                    ZahlstringCol[gx][gy][gz]=Zahlstring[i];
                }
    ...
    for(int ix=0; ix<(Gx); ix++)
    {
	    ...
	    posi=true;
            phase1=true;//n
            inparticle=false;
	    if(Col[ix][2][iz]<0)posi=false;//n
	    for(int iy=2; iy<(Gy-2); iy++)//n
	    {
	        Dx=ix-X;
                if(Dx<0)Dx=-Dx;
	        Dy=iy-Y;
	        ...
	        if(Col[ix][iy][iz]<0)//die Schleife entfernen
		{
		    if(posi)en=iy;
                    posi=false;
                }
	        if(posi)lp=iy;//das entfernen
            }
            my=Col[ix][en][iz]-Col[ix][lp][iz];
            cy=Col[ix][lp][iz]-my*lp;
            Gf[ix][iz]=-cy/my;
            if(inparticle)Gf[ix][iz]=0;
        }
    }
