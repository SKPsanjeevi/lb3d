...
for ($p=0;$p<$nprocs;$p++) {
	open (INFILE,sprintf(".input-file%04d",$p) )
	or die ("Can't open .input-file.");
	printf "Opened \".input-file%04d\"\n",$p;
	$infilename=<INFILE>;	# Fetch the appropriate input-file name.
...
# Note that this parser only works for one pair on each line. FIXME
	...
	# Now write the array back out in AVS-friendly format.
		...
		$outfilename=sprintf("$dir/sites_%s_t%04d_p%04d.all",$gr,$t,$p);
	...
	# Write a field file too.
		...
		$ffilename=sprintf("$dir/sites_%s_t%04d_p%04d.fld",$gr,$t,$p);
		$outfilename=sprintf("sites_%s_t%04d_p%04d.all",$gr,$t,$p);
