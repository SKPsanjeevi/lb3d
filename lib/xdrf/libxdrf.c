...
#define MAXID 20
#define INT32 UINT_MAX
static FILE *xdrfiles[MAXID];
static XDR *xdridptr[MAXID];
static char xdrmodes[MAXID];
static unsigned int cnt;
static int g_gx, g_gy, g_gz, g_nx, g_ny, g_nz;
...
long long int  *pos;
{
unsigned long rocksize=0;
rocksize= (long)g_gx * (long)g_gy * (long)g_gz;
if(rocksize < INT32/4-1)
  *ret = xdr_setpos(xdridptr[*xdrid], (u_int) *pos);
 else{
  *ret = fseek ((FILE *) xdridptr[*xdrid]->x_private, *pos, 0); /*check fseeko*/
 }
}

void
xdrfgetpos_ (xdrid, pos)
int *xdrid;
long long int  *pos;
{
unsigned long rocksize=0;
...
if(rocksize < INT32/4-1)
 ...
 {
  *pos = ftell( (FILE*) xdridptr[*xdrid]->x_private); /*ftello*/
  }
}
...
/*----------------------------------------------------------------------
 VTK output routines follow here.
----------------------------------------------------------------------*/

/*----------------------------------------------------------------------
  Open a vtk data file and write a header (ASCII) - returns a status 
...
char * filename_ptr; int filename_len;
int *datatype;
int *nx;
int *ny;
int *nz;
  ...
  *status = 0;
  ...
  if(fpvtk){
	...
	*fid = (int ) (fpvtk);
    ...
    printf("vtkopen: failed to open file %s\n", filename_ptr);
    *status = 1;
    return;
  }
  ...
  fprintf(fpvtk, "# vtk DataFile Version 2.0\n"
                  "Generated by vtk_io lib\nBINARY\n");
  fprintf(fpvtk, "DATASET STRUCTURED_POINTS\n");
  fprintf(fpvtk, "DIMENSIONS %d %d %d\n", *nx, *ny, *nz);
  fprintf(fpvtk, "SPACING 1 1 1\n");
  fprintf(fpvtk, "ORIGIN 0 0 0\n");
  fprintf(fpvtk, "POINT_DATA %d\n", (*nx)*(*ny)*(*nz));

  /* Switch statement here for datatype? Also allow precision to be
     specified?   (datatype=scalar,2scalar,3scalar,vector) */
  fprintf(fpvtk, "SCALARS scalars float\n");
...
/*----------------------------------------------------------------------
  ...
  Write scalar data to the file (binary) - returns a status value of 0
  if successful.
----------------------------------------------------------------------*/

void vtkfloat_ (fid, numfloats, floatvar, status)
int *fid;
int *numfloats;
float *floatvar;
int *status;
{
  ...
  *status = 0;
  ...
  /* Write the array of floats to file as binary */

  nobj = fwrite(floatvar, sizeof(float), *numfloats, fpvtk);

  if(nobj != *numfloats) *status = 1;

  return;
}

/*----------------------------------------------------------------------
write vector data to the file (binary)
----------------------------------------------------------------------
...
/*----------------------------------------------------------------------
 Close the file - returns 0 if successful
----------------------------------------------------------------------*/
void vtkclose_ (fid, status)
int *fid;
int *status;
{
  FILE *fpvtk;
...
void 
xdrfdarrayset_ (gx, gy, gz, nx, ny, nz, v_size)
int *gx, *gy, *gz, *nx, *ny, *nz, *v_size;
{
	g_gx = *gx;
	g_gy = *gy;
	g_gz = *gz;
...
xdrfdarrayfloat_ (xdrid, fp, ret, x, y, z, nx, ny, nz, pos)
int *xdrid, *ret;
float *fp;
int *x, *y, *z, *nx, *ny, *nz, *pos;
{

	FILE *f = xdrfiles[ *xdrid ];
	long seek_pos;
	...
	*z = g_nz * *nz + *z;
	...
	seek_pos = ((( *x + ( *y * g_gx ) + ( *z * ( g_gy * g_gx ) )  ) * vector_size) + *pos )  * sizeof(float);
...
xdrfdarraydouble_ (xdrid, fp, ret, x, y, z, nx, ny, nz, pos)
int *xdrid, *ret;
double *fp;
int *x, *y, *z, *nx, *ny, *nz, *pos;
{
	...
	*z = g_nz * *nz + *z;
	...
	seek_pos = ((( *x + ( *y * g_gx ) + ( *z * ( g_gy * g_gx ) )  ) * vector_size) + *pos )  * sizeof(double);

	fseek( f, seek_pos, SEEK_SET );

	*ret = xdr_double(xdridptr[*xdrid], fp);
	cnt += sizeof(double);
